...
class Dataset:
    ...
    def __init__(self, name, add_self_loops=False, use_node_embeddings=False,
        ...
        if name in self.tabgraphs_dataset_names:
            graph, features, num_features_mask, targets, train_idx_list, val_idx_list, test_idx_list = \
                self.get_tabgraphs_dataset(name,
                                           use_node_embeddings=use_node_embeddings,
                                           num_features_imputation_strategy=num_features_imputation_strategy,
        ...
        self.num_features_mask = num_features_mask.to(device) if name in self.tabgraphs_dataset_names else None
    ...
    @staticmethod
    def get_tabgraphs_dataset(name, use_node_embeddings, num_features_imputation_strategy, num_features_transform):
        ...
        if num_features.shape[1] > 0:
            ...
            num_features = Dataset.transforms[num_features_transform].fit_transform(num_features)
        ...
        return graph, features, num_features_mask, targets, train_idx_list, val_idx_list, test_idx_list
